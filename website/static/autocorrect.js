...
function initializeAutocorrect(inputId, inputSuggestion) {
                ...
                if (spellCheckXhr.readyState === XMLHttpRequest.DONE) {
                    if (spellCheckXhr.status === 200) {
                        ...
                        if (suggestions.length > 0) {
                            inputSuggestions.innerHTML = ''; // Clear previous suggestions
    ...
    inputField.addEventListener('keydown', function(event) {
        if (event.key === 'Tab' ) {
            ...
            isProgrammaticInput = true; // Set flag to indicate programmatic input
            var inputEvent = new Event('input', {
                bubbles: true,
                cancelable: true,
            });
            this.dispatchEvent(inputEvent);
            isProgrammaticInput = false; // Reset flag after input event
        }
    });
}
});